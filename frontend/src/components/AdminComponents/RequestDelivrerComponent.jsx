...
const RequestDelivrerComponent = ({ papers = [], onEdit, onDelete }) => {
  const providerPapers = papers.filter(
    (paper) => paper.role_requested === 'delivrer' && paper.request_status == 1
  );
    ...
    <div className="bg-white min-h-screen py-12 px-6 flex flex-col items-center w-screen">
      ...
      <div className="w-full max-w-5xl space-y-10">
        <h1 className="text-3xl font-bold text-gray-900 flex gap-5">
          ...
          <p className="text-yellow-500">Livreur</p>
        </h1>
              ...
              {providerPapers.map((paper) => (
                <tr key={paper.id}>
                  ...
                  <td className="px-6 py-4">
                    <a
                      className="text-yellow-500 underline"
                      href={`/admin/request/provider/image/${paper.driverLicense}`}
                      target="_blank"
                      rel="noopener noreferrer"
                    >
                      Voir le document
...
export default RequestDelivrerComponent;
